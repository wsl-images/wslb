param(
    [string]$Out = "wsl-images-oobe-matrix-report.json"
)
...
try {
    $env:WSLB_OOBE_MATRIX = "1"
    $tmpLog = [System.IO.Path]::GetTempFileName()
    try {
        & go test -json ./internal/targets/wsl -run TestWSLImagesOOBEMatrix 2>&1 | Tee-Object -FilePath $tmpLog | Out-Host
        $exitCode = $LASTEXITCODE
    } finally {
        Remove-Item Env:WSLB_OOBE_MATRIX -ErrorAction SilentlyContinue
    }
    ...
    foreach ($line in $lines) {
        ...
        if (-not $obj.Test.StartsWith("TestWSLImagesOOBEMatrix/")) { continue }
        $image = $obj.Test.Substring("TestWSLImagesOOBEMatrix/".Length)
    ...
    Write-Host "OOBE matrix report: $Out"
